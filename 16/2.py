...
class Packet:
    ...
    def version_sum(self):
        ...
        return self.version + sum([s.version_sum() for s in self.subs])

    def calc(self):
        if self.type_id == 4:
            return self.value

        l = [s.calc() for s in self.subs]
...
def test():
    examples = [(line.strip().split(":")[0], int(line.strip().split(":")[1])) for line in
                open("input_example_2").readlines()]

    for example, result in examples:
        p = Packet(example, True)
        print("Test %s\n%s: Expected: %s - Got: %s" % (
            example,
            f"\033[1m\033[92mPASS\033[0m" if result == p.calc() else f"\033[1m\033[91mFAIL\033[0m",
            result,
            p.calc()
        ), end="\n\n")
